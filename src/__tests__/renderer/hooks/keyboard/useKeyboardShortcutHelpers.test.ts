...
describe('unbound shortcuts (keys: [])', () => {
	...
	const shortcuts: Record<string, Shortcut> = {
		...
		paneUnbound: { id: 'paneUnbound', label: 'Pane Unbound', keys: [] },
	};

	const render = () =>
		renderHook(() => useKeyboardShortcutHelpers({ shortcuts, tabShortcuts: shortcuts }));
	...
	it.each([
		...
		['Enter', { key: 'Enter' }],
		['Space', { key: ' ' }],
		['a modified key', { key: 'k', metaKey: true }],
		['a shifted key', { key: 'K', shiftKey: true }],
		['an alt combo', { key: 'a', altKey: true, code: 'KeyA' }],
	])('isShortcut never matches an unbound action for %s', (_label, opts) => {
		const { result } = render();
		expect(result.current.isShortcut(createKeyboardEvent(opts), 'tileAiBelow')).toBe(false);
	});

	it('isTabShortcut never matches an unbound action', () => {
		const { result } = render();
		expect(result.current.isTabShortcut(createKeyboardEvent({ key: 'a' }), 'tileAiBelow')).toBe(
			false
		);
	});

	it('isPaneShortcut never matches an unbound action', () => {
		const { result } = render();
		const event = createKeyboardEvent({ key: 'd', metaKey: true, ctrlKey: true });
		expect(result.current.isPaneShortcut(event, 'paneUnbound')).toBe(false);
	});

	it('still matches a bound action, so the guard is not over-broad', () => {
		const { result } = render();
		expect(
	...
	it('matches once the user records a binding for a previously unbound action', () => {
		const bound: Record<string, Shortcut> = {
			tileAiBelow: {
				id: 'tileAiBelow',
				label: 'Tile New AI Chat Below',
				keys: ['Meta', 'Shift', 'y'],
		...
		const { result } = renderHook(() =>
			useKeyboardShortcutHelpers({ shortcuts: bound, tabShortcuts: {} })
		);
		const event = createKeyboardEvent({ key: 'y', metaKey: true, shiftKey: true });
		expect(result.current.isShortcut(event, 'tileAiBelow')).toBe(true);
	});
});
