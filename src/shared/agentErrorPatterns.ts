 ...
 * It is THE bank: main parses streaming agent output through it, and the wizard
 * classifies a finished agent run through it. A second bank drifts - the wizard
 * used to carry its own hand-written auth regexes that had already fallen behind
 * the ones here, so it told users to run a login command for a provider that was
 * never claude-code.
 *
 * It lives in `shared/` and stays free of Node builtins for that reason: the
 * renderer bundle has no `fs`/`os` polyfill, so a single main-process import
 * would make it throw on load there. Main wraps it in
 * `src/main/parsers/error-patterns.ts`, which installs the log sink below.
 *
 * Usage:
 * ```typescript
...
/**
 * Where this module's diagnostics go, when anything is listening.
 *
 * The bank is shared, so it cannot import either process's logger. Main installs
 * a sink on import (see the wrapper), which keeps every existing log line
 * exactly where it was; the renderer installs nothing and the calls are no-ops.
 */
export type ErrorPatternLogSink = (
	level: 'warn' | 'debug' | 'info',
	message: string,
	data?: Record<string, unknown>
) => void;

let logSink: ErrorPatternLogSink | null = null;

/** Route this module's diagnostics somewhere. Pass `null` to silence them. */
export function setErrorPatternLogSink(sink: ErrorPatternLogSink | null): void {
	logSink = sink;
}
...
// An auth message states WHAT failed and stops there. The remedy belongs to
...
const CLAUDE_ERROR_PATTERNS: AgentErrorPatterns = {
	auth_expired: [
		...
		},
		{
			...
			message: 'Authentication failed. Sign in again to continue.',
			recoverable: true,
		},
		{
			pattern: /authentication_error/i,
			message: 'Authentication error. Sign in again to continue.',
			recoverable: true,
		},
		{
		...
		},
		{
			...
			message: 'The session expired and the provider asked for a new login.',
			recoverable: true,
		},
		{
		...
		},
		{
			...
			message: 'Authentication failed (401). Sign in again to continue.',
			recoverable: true,
		},
		{
...
const COPILOT_ERROR_PATTERNS: AgentErrorPatterns = {
	auth_expired: [
		{
			pattern: /authentication failed/i,
			message: 'Authentication failed. Sign in to GitHub again to continue.',
			recoverable: true,
		},
		{
			pattern: /not authenticated/i,
		...
		},
		{
			...
			message: 'The GitHub token was rejected. Sign in again to continue.',
			recoverable: true,
		},
	],
...
export function getErrorPatterns(agentId: ToolType | string): AgentErrorPatterns {
	...
	if (!isValidAgentId(agentId)) {
		log('warn', `getErrorPatterns: Unknown agent ID "${agentId}".`);
	}
		...
		for (const pattern of typePatterns) {
			...
			if (match) {
				...
				if (
					...
					log('info', '[ErrorPatterns] Shell parse error detected', {
						errorType,
